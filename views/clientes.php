    ...
    $sql = "
        SELECT 
            ...
            COALESCE((
                SELECT COUNT(*) 
                FROM ventas 
                WHERE cliente_id = c.id AND tipo_entrega = 'Domicilio'
...
$msj = request('msj');
if ($msj === 'cliente_creado') {
    $msg = '✅ Cliente creado exitosamente.';
} elseif ($msj === 'cliente_eliminado') {
    $msg = '✅ Cliente eliminado correctamente.';
} elseif ($msj === 'estado_actualizado') {
    $msg = '✅ Estado del cliente actualizado.';
}

$err = request('error');
if ($err === 'cliente_tiene_pedidos') {
    $error = '⚠️ No se puede eliminar: tiene pedidos asociados.';
} elseif ($err === 'id_invalido') {
    $error = '⚠️ ID de cliente inválido.';
}
    ...
    <main class="main-content-panel">
        ...
        <?php if (empty($search)): ?>
            <div class="empty-state">
                ...
                <p>Introduce el nombre, NIT o cédula en la barra de búsqueda.</p>
                        ...
                        <?php if (count($res_clientes) > 0): ?>
                            <?php foreach ($res_clientes as $cli): 
                                ...
                                if ($ultimaCompra) {
                                    ...
                                    $esInactivoPorTiempo = ($diasSinComprar > 365);
                                } else {
                                    $diasSinComprar = -1;
                                }
                            ?>
                                <tr class="<?= $esInactivoPorTiempo ? 'fila-inactiva' : '' ?>">
                                    ...
                                    <td class="actions-cell">
                                        ...
                                        <?php if ($cli['total_domicilios'] > 0): ?>
                                            <span class="badge badge-info" 
                                            title="<?= $cli['total_domicilios'] ?> domicilios realizados">
                                                🏠 <?= $cli['total_domicilios'] ?>
                                            </span>
                                        <?php endif; ?>
                                        ...
                                        <?php if ($_SESSION['role'] === 'admin'): ?>
                                            <?php if (($cli['estado'] ?? 'activo') === 'activo'): ?>
                                                ...
                                                onclick="return confirm('¿Desactivar este cliente?')">⏸️</a>
                                            <?php else: ?>
...
.page-header h1 { color: #1e293b; font-size: 1.6rem; font-weight: 700; margin: 0; }
.page-header p { color: #64748b; margin: 5px 0 0 0; font-size: 0.95rem; }

.alert-success {
    padding: 12px 16px; background: #d1fae5; color: #065f46;
    border-left: 4px solid #10b981; border-radius: 6px; margin-bottom: 20px; font-weight: 500;
}
.alert-error {
    padding: 12px 16px; background: #fee2e2; color: #991b1b;
    border-left: 4px solid #ef4444; border-radius: 6px; margin-bottom: 20px; font-weight: 500;
}
.alert-warning {
    padding: 12px 16px; background: #fef3c7; color: #92400e;
    border-left: 4px solid #f59e0b; border-radius: 6px; margin-bottom: 20px; font-weight: 500;
}
...
.search-clear {
    ...
    text-decoration: none; color: #94a3b8; font-weight: bold; font-size: 1.2rem;
}
...
.empty-icon { font-size: 3.5rem; display: block; margin-bottom: 15px; }
.empty-state h2 { color: #1e293b; margin-bottom: 8px; font-size: 1.4rem; }
.empty-state p { color: #64748b; max-width: 500px; margin: 0 auto 20px; line-height: 1.5; }
...
.data-table th { padding: 14px; text-align: left; color: #475569; font-weight: 600; font-size: 0.9rem; }
.data-table td { padding: 14px; border-bottom: 1px solid #e2e8f0; color: #334155; vertical-align: top; }
...
.actions-cell {
    text-align: center; display: flex; gap: 6px;
    align-items: center; justify-content: center; flex-wrap: wrap;
}
.btn-action {
    font-size: 1.1rem; text-decoration: none; padding: 4px 6px;
    border-radius: 4px; transition: background 0.2s;
}
.btn-action:hover { background: #f1f5f9; }

.no-results { text-align: center; padding: 40px !important; color: #94a3b8; }
.no-results .empty-icon { font-size: 2rem; margin-bottom: 10px; display: block; }
...
.btn-primary {
    ...
    font-size: 0.95rem; text-decoration: none; display: inline-block; transition: background 0.2s;
}
.btn-primary:hover { background: #1d4ed8; }
.btn-secondary {
    padding: 10px 20px; background: white; color: #475569;
    border: 1px solid #cbd5e1; border-radius: 6px; font-weight: 600; cursor: pointer;
    font-size: 0.95rem; text-decoration: none; display: inline-block; transition: all 0.2s;
}
...
@media (max-width: 768px) {
    ...
    .fila-inactiva { background: #fef3c7 !important; opacity: 0.85; }
    .fila-inactiva:hover { background: #fde68a !important; }
}
</style>
